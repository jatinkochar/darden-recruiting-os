...
type EventWithExtra = RecruitingEvent & { userNotes?: string; user_notes?: string; emailBody?: string; email_body?: string };
...
function scoreEvent(event: RecruitingEvent) {
  return (event.status === "Registered" ? 30 : 0) + (event.meetingLink ? 20 : 0) + (event.registrationLink ? 10 : 0) + (event.passcode ? 8 : 0) + (!/reminder|confirmed to attend/i.test(event.title) ? 10 : 0);
}
...
function displayDate(event: RecruitingEvent, timezone: string) {
  ...
  return new Intl.DateTimeFormat(undefined, { weekday: "short", month: "short", day: "numeric", timeZone: timezone === "local" ? undefined : timezone }).format(d);
}
    ...
    <div className="space-y-5">
      <div className="rounded-[32px] border border-stone-200 bg-white p-7 shadow-medium">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-center">
          <div>
            <div className="text-xs font-black uppercase tracking-[0.18em] text-[#E57200]">Events · {mode}</div>
            <h1 className="mt-2 text-4xl font-black tracking-tight text-[#172033]">Events</h1>
            <p className="mt-2 max-w-2xl text-sm font-medium text-stone-600">Browse, filter, and manage recruiting sessions without the Gmail chaos.</p>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        <StatCard label="Visible" value={filtered.length} tone="navy" />
        <StatCard label="All Events" value={displayEvents.length} />
        <StatCard label="Upcoming" value={upcoming.length} tone="orange" />
        <StatCard label="Hidden Dups" value={Math.max(events.length - displayEvents.length, 0)} tone="blue" />
      </section>

      <section className="rounded-[28px] border border-stone-200 bg-white p-5 shadow-soft">
        <div className="mb-4 flex items-center gap-2 text-sm font-black text-[#172033]"><SlidersHorizontal size={16} /> Filters</div>
        <div className="mb-4">
          <label className="label">Search</label>
          <div className="relative">
            <Search className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-stone-400" size={18} />
            <input className="input pl-12" placeholder="Search events, companies, notes, links..." value={query} onChange={(e) => setQuery(e.target.value)} />
          </div>
        </div>
        <div className="grid gap-3 md:grid-cols-6">
          <FilterSelect label="Date" value={dateFilter} onChange={(value) => setDateFilter(value as DateFilter)}><option value="upcoming">Upcoming</option><option value="today">Today</option><option value="next7">Next 7 days</option><option value="next30">Next 30 days</option><option value="missing">Missing date</option><option value="ended">Ended</option><option value="all">All dates</option></FilterSelect>
          <FilterSelect label="Company" value={company} onChange={setCompany}><option value="">All companies</option>{companies.map((c) => <option key={c}>{c}</option>)}</FilterSelect>
          <FilterSelect label="Status" value={status} onChange={setStatus}><option value="">All statuses</option>{statuses.map((s) => <option key={s}>{s}</option>)}</FilterSelect>
          <FilterSelect label="Timezone" value={timezone} onChange={setTimezone}>{TIMEZONES.map((tz) => <option key={tz.value} value={tz.value}>{tz.label}</option>)}</FilterSelect>
          <FilterSelect label="Duplicates" value={duplicateMode} onChange={(value) => setDuplicateMode(value as DuplicateMode)}><option value="hide">Hide duplicates</option><option value="show">Show duplicates</option></FilterSelect>
          <div>
            <label className="label">View</label>
            <div className="flex rounded-2xl border border-stone-200 bg-stone-50 p-1">
              <button aria-label="Card view" className={`flex flex-1 items-center justify-center rounded-xl px-3 py-2 ${viewMode === "cards" ? "bg-[#232D4B] text-white" : "text-stone-500"}`} onClick={() => setViewMode("cards")}><Grid2X2 size={18} /></button>
              <button aria-label="List view" className={`flex flex-1 items-center justify-center rounded-xl px-3 py-2 ${viewMode === "list" ? "bg-[#232D4B] text-white" : "text-stone-500"}`} onClick={() => setViewMode("list")}><List size={18} /></button>
            </div>
          </div>
        </div>
      </section>
      ...
      {viewMode === "cards" ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{filtered.map((event) => <EventCard key={event.id} event={event} timezone={timezone} onOpen={() => setSelected(event)} onEdit={() => setEditing(event)} onDelete={() => deleteEvent(event.id)} />)}</div>
      ) : (
        <div className="overflow-hidden rounded-[28px] border border-stone-200 bg-white shadow-soft">
          <table className="w-full text-left text-sm">
            <thead className="border-b border-stone-200 bg-stone-50 text-xs font-black uppercase tracking-wider text-stone-500"><tr><th className="p-4">Company</th><th className="p-4">Event</th><th className="p-4">Date</th><th className="p-4">Time</th><th className="p-4">Status</th><th className="p-4">Actions</th></tr></thead>
            <tbody>{filtered.map((event) => <tr key={event.id} className="cursor-pointer border-b border-stone-100 hover:bg-stone-50" onClick={() => setSelected(event)}><td className="p-4 font-black">{event.company}</td><td className="p-4"><div className="font-black text-[#172033]">{event.title}</div><div className="text-xs font-bold text-stone-500">{event.type}</div></td><td className="p-4 font-bold">{displayDate(event, timezone)}</td><td className="p-4 font-bold">{displayTime(event)}</td><td className="p-4"><span className="pill bg-stone-100 text-stone-700">{computedStatus(event)}</span></td><td className="p-4" onClick={(e) => e.stopPropagation()}><div className="flex gap-2">{event.meetingLink ? <a className="btn px-3 py-2 text-xs" href={event.meetingLink} target="_blank">Join</a> : null}{event.registrationLink ? <a className="btn-secondary px-3 py-2 text-xs" href={event.registrationLink} target="_blank">Register</a> : null}</div></td></tr>)}</tbody>
          </table>
        </div>
      )}

      {filtered.length === 0 ? <div className="rounded-[28px] border border-dashed border-stone-200 bg-white p-10 text-center text-stone-500 shadow-soft">No events found for these filters.</div> : null}
...
function FilterSelect({ label, value, onChange, children }: { label: string; value: string; onChange: (value: string) => void; children: React.ReactNode }) {
  return <label className="block"><span className="label">{label}</span><select className="input w-full min-w-0" value={value} onChange={(e) => onChange(e.target.value)}>{children}</select></label>;
}
