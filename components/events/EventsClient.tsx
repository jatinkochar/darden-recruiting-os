...
export function EventsClient({ seedEvents }: { seedEvents: RecruitingEvent[] }) {
  ...
  async function loadFromSupabase() {
    ...
    // First-run fix:
    // If Supabase is connected but empty, seed it from bundled Gmail/manual events
    // instead of replacing the UI with an empty list.
  ...
  const companies = useMemo(
    () => Array.from(new Set(events.map((e) => e.company).filter(Boolean))).sort(),
    [events]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return events
      .filter((event) => (!company || event.company === company) && (!q || Object.values(event).join(" ").toLowerCase().includes(q)))
      .sort((a, b) => {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-2 text-stone-600">
              Persistent CRUD. Uses Supabase when env vars are configured; otherwise browser storage.
            </p>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard label="Registered" value={events.filter((e)=>e.status==="Registered").length}/>
        <StatCard label="To Register" value={events.filter((e)=>["Register","Manual Entry"].includes(e.status)).length}/>
      </section>

      <section className="card grid gap-3 p-4 md:grid-cols-[1fr_220px]">
      ...
      {filtered.length === 0 ? (
        <div className="card p-10 text-center text-stone-500">No events found.</div>
      ) : null}
