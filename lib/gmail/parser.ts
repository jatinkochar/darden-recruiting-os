...
const EVENT = [
  ...
  /networking/i, /panel/i, /workshop/i
];
...
function normalizeSubject(subject: string) {
  return subject
    .replace(/^(re|fw|fwd):\s*/gi, "").replace(/^reminder:\s*/i, "")
    .replace(/[-–]\s*event reminder\s*$/i, "").replace(/\b24\s*hour reminder:?\s*/i, "")
    .replace(/^confirmed:\s*/i, "").replace(/^you're registered for\s*/i, "")
    .replace(/^you are registered for\s*/i, "").replace(/^thank you for registering for\s*/i, "")
    .replace(/\s+/g, " ").trim();
}
...
function company(text: string) {
  ...
  if (s.includes("bain") || s.includes("@bain.com") || s.includes("atbain.co")) return "Bain";
...
function eventType(text: string) {
  ...
  if (s.includes("office") || s.includes("spotlight")) return "Office Spotlight";
...
function passcode(text: string) {
  const m = text.match(/passcode:\s*([A-Za-z0-9!@#$%^&*_-]+)/i) || text.match(/password:\s*([A-Za-z0-9!@#$%^&*_-]+)/i);
  return m?.[1] || "";
}
function date(text: string) {
  ...
  for (const p of patterns) {
    ...
    if (!m) continue;
    if (p === patterns[3]) return m[0];
    const d = new Date(m.slice(1).join(" ").replace(/^(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)\s+/i, ""));
...
function notes(body: string) {
  ...
  const linkList = urls(plain).slice(0, 5);
  const useful = plain.split("\n").map(l=>l.trim()).filter(Boolean)
    .filter(l=>!/^view in browser/i.test(l)).filter(l=>!/^unsubscribe/i.test(l)).filter(l=>!/^copyright/i.test(l)).slice(0,12);
  return [
...
export function parseRecruitingEmail(input: EmailInput): RecruitingEvent | null {
  ...
  const status = /confirmed|registered|admitted|you.?re registered|thank you for registering/i.test(text) ? "Registered" :
    /invite|invitation/i.test(text) ? "Invite Found" :
    /register|rsvp|sign up/i.test(text) ? "Register" : "Invite Found";
  return {
    id: stableId(c, title, d, st),
    title, company: c, type: eventType(text), date: d, startTime: st, endTime: endTime(text), timezone: timezone(text),
